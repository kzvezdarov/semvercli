...
fn main() {
    let matches = App::new("version-bump")
        ...
        .arg(Arg::with_name("version")
             ...
             .help("Set VERSION")
             .takes_value(true))
        .group(ArgGroup::with_name("versions")
    ...
    let mut manifest_document = manifest_file.parse::<Document>().expect("Invalid Cargo.toml");

    let version_str = manifest_document["package"]["version"].as_str().unwrap();
    let mut version = Version::parse(version_str)
        .expect(&format!("Invalid package version: {} in Cargo.toml", version_str));
